...
export async function generateStaticParams() {
  const slugs = await client.withConfig({ useCdn: false }).fetch(EVENT_SLUGS_QUERY);
  return slugs;
}

export const events = await sanityFetch({
  query: EVENT_QUERY,
  params: { slug: 'your-slug-value' },
  tags: [`event:your-slug-value`, 'venue', 'headline'],
  // revalidate: 3600,
});
...
// Optional: Home Page handler (commented)
/*
export default async function Page() {
  const page = await sanityFetch({
